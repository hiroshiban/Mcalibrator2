classdef optical
  ...
  properties (Hidden) %(SetAccess = protected);
    ...
    DEVICE_OPTICAL_COM1  = 1;
    DEVICE_OPTICAL_COM2  = 2;
    DEVICE_OPTICAL_COM3  = 3;
    DEVICE_OPTICAL_COM4  = 4;
    CALIB_OK             = 0;
    CALIB_COMMERROR      = 1;
    CALIB_OVERFLOW       = 2;
  ...
  methods
    ...
    % constructor
    function obj=optical(port_name)
    ...
    function obj=gen_port(obj,port_name) %#ok
      ...
      if obj.init_flg==1
        disp('serial connection with OptiCAL is already established');
      else
        disp('starting serial communication with OptiCAL');

        % open a serial port
        if strcmp(obj.rscom,'COM1')
          ErrorCode=calllib('Calibrator','calInitialise',obj.DEVICE_OPTICAL_COM1);
        elseif strcmp(obj.rscom,'COM2')
          ErrorCode=calllib('Calibrator','calInitialise',obj.DEVICE_OPTICAL_COM2);
        elseif strcmp(obj.rscom,'COM3')
          ErrorCode=calllib('Calibrator','calInitialise',obj.DEVICE_OPTICAL_COM3);
