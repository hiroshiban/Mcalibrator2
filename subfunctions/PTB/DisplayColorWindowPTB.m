...
% Last Update: "2013-07-05 19:08:56 ban"
...
try
  ...
  if nargin<3 || isempty(fig_id)
    ...
    % open PTB window
    [ptbwindow,ptbrect]=Screen(scr_num-1,'OpenWindow',[255,255,255],scrpos,8); % decrement 1 from scr_num as PTB screen ID starts from 0.
  else
    if ~isempty(fig_id)
      ptbwindow=fig_id;
      ptbrect=Screen('Rect',ptbwindow);
    else
      error('PTB screen corresponding to fig_is is not opend.');
    end
  end
