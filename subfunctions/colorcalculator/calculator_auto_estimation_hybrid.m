function calculator_auto_estimation_hybrid(hObject, eventdata, handles)

% function calculator_auto_estimation_hybrid(hObject, eventdata, handles)
%
% This is a wrapper to AutoColorEstimateHybrid function.
...
% see AutoColorEstimateHybrid.m in subfunctions/mains directory.
%
%
...
hybrid_estimate=AutoColorEstimateHybrid(rawxyY,myxyY,myphosphers,flare_XYZ,lut,colorimeterhandler,displayhandler,options);
...
for mm=1:1:size(myxyY,2)
  ...
  PlotCIE1931xy(hybrid_estimate{mm}.final_xyY(1:2),phosphers,0,1,1,0);
...
str_xyY=[]; str_rgb=[];
for mm=1:1:size(myxyY,2)
  str_xyY=[str_xyY,num2str(hybrid_estimate{mm}.final_xyY(1)),',',num2str(hybrid_estimate{mm}.final_xyY(2)),',',num2str(hybrid_estimate{mm}.final_xyY(3)),';']; %#ok
  if get(handles.use_LUT_radiobutton,'Value')
    str_rgb=[str_rgb,num2str(hybrid_estimate{mm}.final_LUT(1)),',',num2str(hybrid_estimate{mm}.final_LUT(2)),',',num2str(hybrid_estimate{mm}.final_LUT(3)),';']; %#ok
  else
    str_rgb=[str_rgb,num2str(hybrid_estimate{mm}.final_RGB(1)),',',num2str(hybrid_estimate{mm}.final_RGB(2)),',',num2str(hybrid_estimate{mm}.final_RGB(3)),';']; %#ok
  end
end
...
% save the results
eval(sprintf('save %s hybrid_estimate -append;',save_fname));
