function calculator_auto_estimation_lincoeff(hObject, eventdata, handles)

% A wrapper function of AutoColorEstimateLinearCoeff to call it from Mcalibrator2 with GUI-based data handling.
% function calculator_auto_estimation_lincoeff(hObject, eventdata, handles)
%
% This is a wrapper to AutoColorEstimateLinearRatioAdjust function.
...
% Last Update: "2013-12-11 17:39:03 ban (ban.hiroshi@gmail.com)"
...
ratioadjust_estimate=AutoColorEstimateLinearCoeff(rawxyY,myxyY,myphosphors,lut,colorimeterhandler,displayhandler,options);
...
for mm=1:1:size(myxyY,2)
  ...
  PlotCIE1931xy(ratioadjust_estimate{mm}.final_xyY(1:2),phosphors,0,1,1,0);
...
str_xyY=[]; str_rgb=[];
for mm=1:1:size(myxyY,2)
  str_xyY=[str_xyY,num2str(ratioadjust_estimate{mm}.final_xyY(1)),',',num2str(ratioadjust_estimate{mm}.final_xyY(2)),',',num2str(ratioadjust_estimate{mm}.final_xyY(3)),';']; %#ok
  if get(handles.use_LUT_radiobutton,'Value')
    str_rgb=[str_rgb,num2str(ratioadjust_estimate{mm}.final_LUT(1)),',',num2str(ratioadjust_estimate{mm}.final_LUT(2)),',',num2str(ratioadjust_estimate{mm}.final_LUT(3)),';']; %#ok
  else
    str_rgb=[str_rgb,num2str(ratioadjust_estimate{mm}.final_RGB(1)),',',num2str(ratioadjust_estimate{mm}.final_RGB(2)),',',num2str(ratioadjust_estimate{mm}.final_RGB(3)),';']; %#ok
  end
end
