function calculator_auto_estimation_linear(hObject, eventdata, handles)

% A wrapper function of AutoColorEstimateLinear to call it from Mcalibrator2 with GUI-based data handling.
% function calculator_auto_estimation_linear(hObject, eventdata, handles)
%
...
% Last Update: "2013-12-11 17:37:42 ban (ban.hiroshi@gmail.com)"
...
for mm=1:1:size(myxyY,2)
  ...
  PlotCIE1931xy(linear_estimate{mm}.final_xyY(1:2),phosphors,0,1,1,0);
...
str_xyY=[]; str_rgb=[];
for mm=1:1:size(myxyY,2)
  str_xyY=[str_xyY,num2str(linear_estimate{mm}.final_xyY(1)),',',num2str(linear_estimate{mm}.final_xyY(2)),',',num2str(linear_estimate{mm}.final_xyY(3)),';']; %#ok
  if get(handles.use_LUT_radiobutton,'Value')
    str_rgb=[str_rgb,num2str(linear_estimate{mm}.final_LUT(1)),',',num2str(linear_estimate{mm}.final_LUT(2)),',',num2str(linear_estimate{mm}.final_LUT(3)),';']; %#ok
  else
    str_rgb=[str_rgb,num2str(linear_estimate{mm}.final_RGB(1)),',',num2str(linear_estimate{mm}.final_RGB(2)),',',num2str(linear_estimate{mm}.final_RGB(3)),';']; %#ok
  end
end
...
% save the results
eval(sprintf('save %s linear_estimate -append;',save_fname));
