...
%
% [output]
% nonlinear_estimate : cell structure {n x 1}, holding the estimation results with the variables below
%                      .method --- 'LUT' or 'RGB
%                      .wanted_xyY
%                      .measured_xyY
...
if nargin<7 || isempty(options)
  options=optimset('Display','iter','MaxFunEvals',30,'TolFun',1e-3,'TolX',1e-3,'MaxIter',50);
end

% initialize variable to store the results
nonlinear_estimate=cell(size(myxyY,2),1);
...
for mm=1:1:size(myxyY,2)
  ...
  % store the data
  if ~isempty(lut)
    nonlinear_estimate{mm}.method='LUT';
  else
    nonlinear_estimate{mm}.method='RGB';
  end
  nonlinear_estimate{mm}.wanted_xyY=rawxyY(:,mm);
  nonlinear_estimate{mm}.measured_xyY=mxyY;
  nonlinear_estimate{mm}.residuals=mxyY-rawxyY(:,mm);
  nonlinear_estimate{mm}.rms=rms;
  nonlinear_estimate{mm}.RGB=RGB;
  if ~isempty(lut)
    for nn=1:1:3
      [dummy,idx]=min(abs(lut(:,nn)-RGB(nn)));
      nonlinear_estimate{mm}.LUT(nn)=idx;
    end
  end
  nonlinear_estimate{mm}.final_xyY=nonlinear_estimate{mm}.measured_xyY;
  nonlinear_estimate{mm}.final_RGB=nonlinear_estimate{mm}.RGB;
  if ~isempty(lut), nonlinear_estimate{mm}.final_LUT=nonlinear_estimate{mm}.LUT; end
